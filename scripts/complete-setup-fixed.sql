-- Complete Admin Database Setup Script (Fixed)
-- Run this in your Supabase SQL editor

-- 1. Ensure admin_users table has all required columns and proper structure
ALTER TABLE admin_users 
ADD COLUMN IF NOT EXISTS password_hash TEXT,
ADD COLUMN IF NOT EXISTS full_name VARCHAR(255);

-- Fix admin_users id column to be auto-incrementing
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        SELECT setval('admin_users_id_seq', COALESCE((SELECT MAX(id) FROM admin_users), 0) + 1);
    END IF;
END $$;
...
-- 9. Insert the admin user with password (now with auto-generated id)
