...
import { MapPin, Search, Phone, Clock, Package, Navigation, Globe, Star, Car, Route, X, Locate } from "lucide-react";
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div>
                  ...
                  <p className="font-semibold text-lg">{routeInfo.distance}</p>
                </div>
                <div>
                  <p className="text-sm text-foreground/60">Driving Time</p>
                  <p className="font-semibold text-lg">{routeInfo.duration}</p>
                </div>
                <div className="md:col-span-2">
                  <p className="text-sm text-foreground/60">Address</p>
                  <p className="font-semibold text-sm">{nearestStore.address}</p>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              searchResults.map((store) => (
                <Card
                  ...
                  className={`cursor-pointer transition-all ${
                    ...
                    nearestStore?.id === store.id ? "ring-2 ring-green-500 shadow-lg" : "hover:shadow-lg"
                  }`}
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-lg flex items-center gap-2">
                          {store.name}
                          {nearestStore?.id === store.id && (
                            <Badge className="bg-green-600 text-white">Nearest</Badge>
                          )}
                        </CardTitle>
                        <CardDescription className="mt-1">{store.address}</CardDescription>
                      </div>
                      <MapPin className={`h-5 w-5 ${
                        selectedStore?.id === store.id || nearestStore?.id === store.id ? 'text-green-500' : 'text-primary'
                      }`} />
                    </div>
                  </CardHeader>
                  <CardContent>
...
export default StoreLocatorWithRouting;