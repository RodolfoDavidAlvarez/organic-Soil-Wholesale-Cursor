  ...
  useEffect(() => {
    if (products.length > 0) {
      ...
      if (activeCategory === "all") {
        ...
        // Map category IDs to actual product types
        const categoryMapping: { [key: string]: string[] } = {
          compost: ["Compost"],
          amendment: ["Amendment", "Internal"], // Map Internal type to amendment category
          potting: ["Potting"],
          specialty: ["Specialty"],
        };

        const productTypes = categoryMapping[activeCategory] || [activeCategory];
        setFilteredProducts(products.filter((product) => productTypes.some((type) => product.category.toLowerCase() === type.toLowerCase())));
      }
    }
